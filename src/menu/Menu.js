...
function Menu({ startingItems }) {
  ...
  // use "set" to name the 2nd item in state
  ...
  function updateQty(itemId, newQty){
    ...
    const newItems = items.map(i => {
      ...
      if (i.id === itemId) {
        ...
        // use ellipsis (...) to use "spread" technique
    ...
    // Setting the new array value (AKA "newItems" variable) that was returned from the map method as the new state value for "items". Also, because the "setItems" state function is used to update the value for "items", the Menu component re-renders which causes the three MenuItem component to be created (AKA Rendered) again as well.
    setItems(newItems);
  }
  ...
  return (
    <section className='Menu'>
      <h1 className='Menu-title'>Todd's Pizzeria</h1>
      <div>
        {/* <MenuItem
          type={items[0].type}
          price={items[0].price}
